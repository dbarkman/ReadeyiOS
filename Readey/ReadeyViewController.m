...
@synthesize rate, wordsPerMinute, timer;
...
- (void)displayTimeRemaining:(BOOL)initial
{
	...
	[words setText:[NSString stringWithFormat:@"%u words", wordArraySize - marker]];
}

- (IBAction)nextWord:(id)sender
{
	if (marker < (wordArraySize - 1)) {
		int wordLength = 0;
		NSString *word = @"";
		while (wordLength == 0) {
			marker++;
		...
		[self displayTimeRemaining:NO];
	} else if (marker == (wordArraySize - 1)) {
		[timer invalidate];
